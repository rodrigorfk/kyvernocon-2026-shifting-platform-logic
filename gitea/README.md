...
- **Example-02 Kyverno policies** — `MutatingPolicy` resources from `02-keda-prometheus-address/` (platform config, applied before GitOps tools start reconciling)
...
The manifests are sourced from `../gitops-manifests/` and mounted into the Job via a Kustomize `configMapGenerator`. To update the manifests after changes, run `make push-manifests`.
